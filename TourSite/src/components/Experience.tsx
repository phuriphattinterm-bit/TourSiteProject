...
import { selectTours, deleteTour, fetchTours } from '../store/tourSlice';
                        ...
                        tours.map(tour => (
                            <div key={tour.id} className='w-85 md:w-150 flex flex-col gap-2'>
                                <div className='md:w-150 md:h-90 rounded-3xl overflow-hidden'>
                                    <img onClick={() => toOrderBtn(tour)} src={tour.image_url} alt="" className='h-full w-full object-cover hover:scale-150 transition-all duration-500' />
                                </div>
                                <div className='flex justify-between'>
                                    {/*Left Side*/}
                                    <div className='flex flex-col gap-2 justify-center'>
                                        <h1 className='text-2xl font-bold md:mt-3'>{tour.title}</h1>
                                        <h1 className='text-2xl font-bold text-green-500'>{tour.price} {tour.currency}</h1>
                                        <div className='my-2'>
                                 ...
                                 hover:bg-red-500 hover:text-white transition-all duration-500'>
                                    ...
                                    {isLoggedIn && isAdmin ?
                                        <div className='flex flex-col gap-2 justify-center'>
                                            <button onClick={() => handleEdit(tour)} className='p-2 border border-blue-500 text-blue-500 hover:text-white 
                                        hover:bg-blue-500 transition-all duration-500 
                                        rounded-lg text-xl'> Edit </button>
                                            <button onClick={() => dispatch(deleteTour(tour.id))} className='p-2 border border-red-500 text-red-500 hover:text-white 
                                        hover:bg-red-500 transition-all duration-500
                                        rounded-lg text-xl'> Delete</button>
